...
class BertFlashAttention2(BertSelfAttention):
    ...
    def forward(
        ...
        if head_mask is not None:
            # attention_probs = attention_probs * head_mask
            raise ValueError(
                "A mask was passed to the head_mask parameter, this is currently not support for flash attention 2 "
            )
...
BERT_ATTENTION_CLASSES = {
    ...
    "flash_attention_2": BertFlashAttention2,
}

class BertAttention(nn.Module):
    def __init__(self, config, position_embedding_type=None):
        super().__init__()
        self.self = BERT_ATTENTION_CLASSES[config._attn_implementation](config=config, position_embedding_type=position_embedding_type)
